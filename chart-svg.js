...
function renderNorthIndianSVG(chart, opts = {}) {
  ...
  // Header
  svg += `<text x="${width - 50}" y="38" font-family="Georgia, serif" font-size="18" font-weight="600" fill="${COLORS.ink}" text-anchor="end">Джанма Кундали</text>`;
  svg += `<text x="${width - 50}" y="58" font-family="monospace" font-size="12" fill="${COLORS.gold}" text-anchor="end">t.me/janma_kundali_bot</text>`;
  svg += `<text x="${width/2}" y="112" font-family="Georgia, serif" font-size="30" fill="${COLORS.ink}" text-anchor="middle">${esc(title)}</text>`;
  if (subtitle) {
    svg += `<text x="${width/2}" y="138" font-family="monospace" font-size="14" fill="${COLORS.inkSoft}" text-anchor="middle">${esc(subtitle)}</text>`;
  }
  ...
  for (let h = 1; h <= 12; h++) {
    ...
    if (h === 1) {
      svg += `<text x="${numX}" y="${numY}" font-family="Georgia, serif" font-size="17" font-weight="600" fill="${COLORS.gold}" text-anchor="middle">${signNumber}</text>`;
      // Чёткий бейдж "ASC" вместо мелкой звёздочки
      svg += `<rect x="${cx - 23}" y="${cy - 46}" width="46" height="20" rx="4" fill="${COLORS.gold}"/>`;
      svg += `<text x="${cx}" y="${cy - 32}" font-family="Arial, sans-serif" font-size="11" font-weight="700" fill="${COLORS.parchmentCard}" text-anchor="middle">ASC</text>`;
    } else {
      svg += `<text x="${numX}" y="${numY}" font-family="Georgia, serif" font-size="15" fill="${COLORS.inkSoft}" text-anchor="middle">${signNumber}</text>`;
    }
    ...
    planetsHere.forEach((item, i) => {
      ...
      svg += `<text x="${px}" y="${py}" font-family="Arial, sans-serif" font-weight="700" font-size="${fontSize}" fill="${color}" text-anchor="middle" dominant-baseline="central">${PLANET_SYMBOLS[item.name]}</text>`;
    });
    ...
    planetsHere.forEach((item, i) => {
      ...
      const px = startX + col * colGap;
      const py = startY + row * rowGap;
      svg += `<text x="${px}" y="${py + fontSize * 1.15}" font-family="monospace" font-size="9" fill="${COLORS.inkSoft}" text-anchor="middle">${dmsFromDeg(item.p.sign.degInSign)}</text>`;
    });
  }
  ...
  svg += `<text x="54" y="${legendY}" font-family="Arial, sans-serif" font-size="11" fill="${COLORS.inkSoft}">экзальтация</text>`;
  svg += `<circle cx="180" cy="${legendY - 4}" r="5" fill="${COLORS.own}"/>`;
  svg += `<text x="192" y="${legendY}" font-family="Arial, sans-serif" font-size="11" fill="${COLORS.inkSoft}">своя обитель</text>`;
  svg += `<circle cx="320" cy="${legendY - 4}" r="5" fill="${COLORS.debilitated}"/>`;
  svg += `<text x="332" y="${legendY}" font-family="Arial, sans-serif" font-size="11" fill="${COLORS.inkSoft}">падение</text>`;
  ...
  svg += `<text x="40" y="${listStartY - 6}" font-family="Arial, sans-serif" font-size="11" font-weight="700" fill="${COLORS.inkSoft}">ПЛАНЕТА</text>`;
  svg += `<text x="180" y="${listStartY - 6}" font-family="Arial, sans-serif" font-size="11" font-weight="700" fill="${COLORS.inkSoft}">ЗНАК</text>`;
  svg += `<text x="280" y="${listStartY - 6}" font-family="Arial, sans-serif" font-size="11" font-weight="700" fill="${COLORS.inkSoft}">ГРАДУС</text>`;
  svg += `<text x="400" y="${listStartY - 6}" font-family="Arial, sans-serif" font-size="11" font-weight="700" fill="${COLORS.inkSoft}">НАКШАТРА</text>`;
  ...
  // Строка Лагны (Асцендента) — мягкое выделение, отдельно от 9 планет
  {
    ...
    svg += `<text x="40" y="${y}" font-family="Arial, sans-serif" font-weight="700" font-size="13" fill="${COLORS.gold}">Лагна</text>`;
    svg += `<text x="180" y="${y}" font-family="Arial, sans-serif" font-size="13" fill="${COLORS.ink}">${asc.sign.index + 1}. ${esc(asc.sign.name)}</text>`;
    svg += `<text x="280" y="${y}" font-family="monospace" font-size="12" fill="${COLORS.inkSoft}">${dmsFromDeg(asc.sign.degInSign)}</text>`;
    svg += `<text x="400" y="${y}" font-family="Arial, sans-serif" font-size="13" fill="${COLORS.ink}">${esc(asc.nakshatra.name)} (пада ${asc.nakshatra.pada})</text>`;
  }
  ...
  order.forEach((name, i) => {
    ...
    svg += `<text x="40" y="${y}" font-family="Arial, sans-serif" font-weight="700" font-size="13" fill="${color}">${PLANET_SYMBOLS[name]}</text>`;
    svg += `<text x="70" y="${y}" font-family="Arial, sans-serif" font-size="13" fill="${COLORS.ink}">${esc(name)}</text>`;
    svg += `<text x="180" y="${y}" font-family="Arial, sans-serif" font-size="13" fill="${COLORS.ink}">${p.sign.index + 1}. ${esc(p.sign.name)}</text>`;
    svg += `<text x="280" y="${y}" font-family="monospace" font-size="12" fill="${COLORS.inkSoft}">${dmsFromDeg(p.sign.degInSign)}</text>`;
    svg += `<text x="400" y="${y}" font-family="Arial, sans-serif" font-size="13" fill="${COLORS.ink}">${esc(p.nakshatra.name)} (пада ${p.nakshatra.pada})</text>`;
  });
...
function renderNorthIndianSVGWithTransits(natalChart, transitsResult, opts = {}) {
  ...
  for (let h = 1; h <= 12; h++) {
    ...
    if (transitsHere.length > 0) {
      ...
      transitsHere.forEach((item, i) => {
        ...
        svg += `<text x="${px}" y="${py}" font-family="Arial, sans-serif" font-weight="600" font-size="${tFontSize}" fill="${TRANSIT_COLOR}" text-anchor="middle">${PLANET_SYMBOLS[item.name]} ${dmsFromDeg(item.t.sign.degInSign)}</text>`;
      });
    }
  }
  ...
  svg += `<text x="452" y="${legendY}" font-family="Arial, sans-serif" font-size="11" fill="${COLORS.inkSoft}">транзит</text>`;
  ...
  const listStartY = legendY + 30;
  svg += `<line x1="40" y1="${listStartY - 24}" x2="${width - 40}" y2="${listStartY - 24}" stroke="${COLORS.gold}" stroke-width="1"/>`;
  svg += `<text x="40" y="${listStartY - 6}" font-family="Arial, sans-serif" font-size="11" font-weight="700" fill="${COLORS.inkSoft}">ДОМ</text>`;
  svg += `<text x="100" y="${listStartY - 6}" font-family="Arial, sans-serif" font-size="11" font-weight="700" fill="${COLORS.inkSoft}">ЗНАК</text>`;
  svg += `<text x="220" y="${listStartY - 6}" font-family="Arial, sans-serif" font-size="11" font-weight="700" fill="${COLORS.inkSoft}">НАТАЛ</text>`;
  svg += `<text x="530" y="${listStartY - 6}" font-family="Arial, sans-serif" font-size="11" font-weight="700" fill="${COLORS.inkSoft}">ТРАНЗИТ</text>`;
  ...
  for (let h = 1; h <= 12; h++) {
    ...
    svg += `<text x="40" y="${y}" font-family="Georgia, serif" font-size="13" font-weight="700" fill="${COLORS.gold}">${h}</text>`;
    svg += `<text x="100" y="${y}" font-family="Arial, sans-serif" font-size="12" fill="${COLORS.ink}">${signIdx + 1}. ${esc(SIGN_NAMES[signIdx])}</text>`;
    ...
    const natalStr = natalHere.length
      ...
      : '—';
    svg += `<text x="220" y="${y}" font-family="Arial, sans-serif" font-size="12" fill="${COLORS.ink}">${esc(natalStr)}</text>`;
    ...
    const transitStr = transitHere.length
      ...
      : '—';
    svg += `<text x="530" y="${y}" font-family="Arial, sans-serif" font-size="12" fill="${TRANSIT_COLOR}">${esc(transitStr)}</text>`;
  }
...
function renderSouthIndianSVG(natalChart, opts = {}) {
  ...
  for (const pos of SOUTH_GRID_LAYOUT) {
    ...
    const numY = cy0 + 20;
    if (isAsc) {
      svg += `<text x="${numX}" y="${numY}" font-family="Georgia, serif" font-size="19" font-weight="600" fill="${COLORS.gold}">${pos.sign + 1}</text>`;
      // Чёткий бейдж "ASC" вместо мелкой звёздочки
      const badgeX = cx0 + cellSize - 54;
      const badgeY = cy0 + 8;
      svg += `<rect x="${badgeX}" y="${badgeY}" width="46" height="20" rx="4" fill="${COLORS.gold}"/>`;
      svg += `<text x="${badgeX + 23}" y="${badgeY + 14}" font-family="Arial, sans-serif" font-size="11" font-weight="700" fill="${COLORS.parchmentCard}" text-anchor="middle">ASC</text>`;
    } else {
      svg += `<text x="${numX}" y="${numY}" font-family="Georgia, serif" font-size="17" fill="${COLORS.inkSoft}">${pos.sign + 1}</text>`;
    }
    ...
    planetsHere.forEach((item, i) => {
      ...
      const px = startX + col * colGap;
      const py = startY + row * rowGap;
      svg += `<text x="${px}" y="${py + fontSize * 1.1}" font-family="monospace" font-size="8.5" fill="${COLORS.inkSoft}" text-anchor="middle">${dmsFromDeg(item.p.sign.degInSign)}</text>`;
    });
  }
  ...
  svg += `<text x="${centerX}" y="${centerY - 6}" font-family="Georgia, serif" font-size="16" fill="${COLORS.ink}" text-anchor="middle">Раши</text>`;
  svg += `<text x="${centerX}" y="${centerY + 12}" font-family="monospace" font-size="9" fill="${COLORS.inkSoft}" text-anchor="middle">South Indian</text>`;
